import type { DataKey, Order, StoreOverview } from '#shared/types'
...
export async function buildOverview(api: SallaApi, days = 30): Promise<StoreOverview> {
  ...
  const top = new Map<string, { sold: number; revenue: number }>()
  for (const o of recent) for (const it of o.items) {
    const t = top.get(it.name) ?? { sold: 0, revenue: 0 }
    t.sold += it.quantity
    t.revenue += it.amount
    top.set(it.name, t)
  }
  ...
  return {
    ...
    kpis: {
      ...
      abandonedValue: Math.round(carts.reduce((a, c) => a + c.total.amount, 0)),
      avgRating: reviews.length ? Math.round((reviews.reduce((a, r) => a + r.rating, 0) / reviews.length) * 10) / 10 : null,
      customersCount: customers.length,
    },
    salesByDay: [...byDay].map(([date, total]) => ({ date, total: Math.round(total) })),
    topProducts: [...top]
      .map(([name, t]) => ({ id: products.find(p => p.name === name)?.id ?? name, name, ...t }))
