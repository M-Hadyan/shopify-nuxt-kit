...
const props = defineProps<{ skill: SkillDef; to?: string }>()
const tone = computed(() => TONES[props.skill.tone])
</script>

<template>
  <NuxtLink :to="to ?? `/app/skills/${skill.slug}`" class="card card-hover group relative flex flex-col overflow-hidden p-5">
