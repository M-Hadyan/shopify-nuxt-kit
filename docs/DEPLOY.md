...
في صفحة التطبيق ← **Environment Variables** أضف هذي. خلّ القيم الخاصة بسلة فاضية الحين، وترجع لها في الخطوة ٨.
...
| `NUXT_ADMIN_EMAIL` | إيميلك لدخول لوحة الإدارة (الخطوة ٦) |
| `NUXT_ADMIN_PASSWORD_HASH` | من `npm run admin:setup` (الخطوة ٦) |
| `NUXT_ADMIN_TOTP_SECRET` | من `npm run admin:setup` (الخطوة ٦) |
| `NUXT_ALERT_WEBHOOK_URL` | (اختياري) رابط Webhook لـ Slack أو Discord أو Telegram للتنبيهات |
| `NUXT_ADMIN_TOKEN` | (اختياري) مفتاح للسكربتات، ٢٤ حرف أو أكثر. يفتح واجهات الإدارة عبر هيدر `x-admin-token` |
...
| `NUXT_SALLA_CLIENT_ID` | من سلة (الخطوة ٨) |
| `NUXT_SALLA_CLIENT_SECRET` | من سلة (الخطوة ٨) |
| `NUXT_SALLA_WEBHOOK_SECRET` | من سلة (الخطوة ٨) |
...
## الخطوة ٦: لوحة الإدارة

1. على جهازك داخل مجلد المشروع شغّل:
   ```bash
   npm run admin:setup
   ```
   يسألك عن إيميلك وكلمة مرور (١٢ حرف أو أكثر)، ويطلع لك ٣ قيم.
2. حط القيم الثلاث في متغيرات Coolify: `NUXT_ADMIN_EMAIL`، `NUXT_ADMIN_PASSWORD_HASH`، `NUXT_ADMIN_TOTP_SECRET`، وبعدها Redeploy.
3. أضف السر لتطبيق مصادقة في جوالك (Google Authenticator أو 1Password أو Authy).
4. ادخل `https://DOMAIN/admin` بالإيميل وكلمة المرور والكود.

**وش فيها:**
- **المراقبة:** صحة النظام (قاعدة البيانات، Claude، سلة)، ورسوم آخر ٢٤ ساعة للتشغيلات والأخطاء والتكلفة والطلبات، والإيراد مقابل تكلفة الذكاء الاصطناعي. تتحدث كل ٣٠ ثانية.
- **المتاجر:** قائمة كل المتاجر، وصفحة لكل متجر تقدر منها توقفه، أو تغيّر باقته، أو تهديه تشغيلات.
- **السجلات:** كل الأحداث وكل طلبات الـ API، مع فلترة وبحث ووضع "مباشر".

**التنبيهات (مستحسن):** حط رابط Webhook في `NUXT_ALERT_WEBHOOK_URL`، ويوصلك تنبيه وقت الأخطاء والأحداث الأمنية (مرة كل ٥ دقايق لكل نوع كحد أقصى):
- **Discord:** إعدادات القناة ← Integrations ← Webhooks ← انسخ الرابط.
- **Slack:** Incoming Webhooks.
- **Telegram:** `https://api.telegram.org/bot<TOKEN>/sendMessage?chat_id=<CHAT_ID>`

> السجلات تطلع كمان في **Logs** داخل Coolify بصيغة JSON. وتقدر تفعّل من Coolify تنبيهات لو الحاوية طاحت (Notifications).

---

## الخطوة ٧: تأكد إن الموقع شغال
...
## الخطوة ٨: تطبيق رواج في بوابة شركاء سلة
...
- [ ] تقرير التكاليف يشتغل:
  ...
  ```
- [ ] لوحة الإدارة `https://DOMAIN/admin` تعرض متجرك في "المتاجر"، وتشغيلاتك في "السجلات".
...
| بيانات المتجر ناقصة في النتائج | صلاحية ناقصة في الخطوة ٨، أضفها وأعد تثبيت التطبيق |
| `/admin` يرجع 404 أو الدخول يفشل | متغيرات الأدمن الثلاثة ناقصة، أو ساعة الجوال غير مضبوطة (كود 2FA يعتمد على الوقت) |
| "محاولات كثيرة" في دخول الأدمن | انتظر ١٥ دقيقة. الحد ٥ محاولات لكل IP |
